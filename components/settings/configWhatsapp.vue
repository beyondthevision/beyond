        ...
        <b-card>
            ...
            <div class="fields user-data-container">
                <div class="form-group">
                    ...
                    <el-input type="text"
                        name="instance"
                        v-model="current.senderdata.name"
                        @input="change"
                ...
                <div class="form-group">
                    <el-input 
                        type="text"
                        name="apitoken"
                        v-model="current.senderdata.apitoken"
                        @input="change"
                ...
                <div class="form-group">
                    <el-input type="text"
                        name="instance"
                        v-model="current.senderdata.instance"
                        @input="change"
                        placeholder="INSERT INSTANCE NUMBER"
                    />
                </div>
            </div>
            ...
            <table class="table table-striped">
                ...
                <draggable v-model="allData" tag="tbody" @update="moved">
                    <tr v-for="user in allData" :key="user.id">
                        <td>
                            <button class="btn-danger btn btn-sm" @click="edit(user.senderdata)" type="danger" size="small"><i class="el-icon-edit"></i></button>
                            <el-popconfirm
                        ...
                        <td>{{ user.senderdata.name }}</td>
                        <td>{{ user.senderdata.apitoken }}</td>
                        <td>{{ user.senderdata.instance }}</td>
                    </tr>
                </draggable>
            </table>
    ...
    methods: {
        ...
        async moved (data) {
           this.allData.forEach(async (register, item) => {
                register.order = item
                await updateWhatsappData(register.id, register)
            });
        ...
        updateData () {
            ...
            updateWhatsappData(data.id, data)
            .then(response => {
        ...
        init () {
            this.setAllData()
            .then(response => {
                return countWhatsappNumbers(this.$auth.user.id)
            })
            .then(response => {
        ...
        setAllData () {
            return getAllWhatsapp(this.$auth.user.id, this.pagination)
            .then(response => {
                this.allData = response.data.sort((a, b) => {
                    if (a.senderdata.order > b.senderdata.order) {
                        return 1;
                    }
        ...
        save () {
            let data = this.current.senderdata;
            saveWhatsappData(this.$auth.user.id, data)
            .then(response => {
                this.$notify({
                    title: 'Congratulations',
